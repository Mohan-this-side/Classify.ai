...
export default function ApiKeySettings({ 
  ...
  const testApiKey = async (key: string): Promise<boolean> => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/workflow/test-gemini-key', {
        method: 'POST',
        headers: {
  ...
  const handleApiKeyChange = async (value: string) => {
    ...
    // Test the API key after a short debounce (only test if longer than 10 chars)
    if (value.length > 10) {
      // Test the API key
      ...
      // Don't validate yet, wait for user to finish typing
      setIsValid(null)
    }
  }
