...
class EDAAgent(BaseAgent):
    ...
    async def execute(self, state: ClassificationState) -> ClassificationState:
        try:
            ...
            interactive_plots = await self._generate_interactive_visualizations(df, target_column, session_plots_dir)
            
            # 9. Advanced Statistical Analysis
            statistical_analysis = await self._perform_advanced_statistical_analysis(df, target_column)
            ...
            state["interactive_plots"] = interactive_plots
            state["statistical_analysis"] = statistical_analysis
            state["automated_insights"] = automated_insights
    ...
    async def _analyze_missing_values(self, df: pd.DataFrame, plots_dir: Path) -> Dict[str, Any]:
        ...
        if len(missing_cols) > 0:
            missing_cols.sort_values(ascending=True).plot(
        ...
        missing_pct_cols = missing_percentage[missing_percentage > 0]
        if len(missing_pct_cols) > 0:
            missing_pct_cols.sort_values(ascending=True).plot(
    ...
    async def _analyze_feature_relationships(self, df: pd.DataFrame, target_column: str, plots_dir: Path) -> Dict[str, Any]:
        ...
        for i, feature in enumerate(top_features[:n_rows*n_cols]):
            ...
            # Add trend line
            # Ensure both arrays have the same length by dropping NaN from both simultaneously
            valid_mask = df[feature].notna() & df[target_column].notna()
            if valid_mask.sum() > 1:  # Need at least 2 points for polyfit
                x_vals = df[feature][valid_mask]
                y_vals = df[target_column][valid_mask]
    ...
    def _generate_missing_value_insights(self, missing_summary: pd.Series, missing_percentage: pd.Series) -> List[str]:
        ...
        low_missing = missing_percentage[(missing_percentage > 0) & (missing_percentage <= 10)]
        if len(low_missing) > 0:
            insights.append(f"ℹ️ Low missing values (0-10%): {list(low_missing.index)}")
    ...
    async def _generate_interactive_visualizations(self, df: pd.DataFrame, target_column: str, plots_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(df.select_dtypes(include=[np.number]).columns) > 1:
                ...
                heatmap_file = plots_dir / "interactive_correlation_heatmap.html"
                fig.write_html(str(heatmap_file))
                interactive_plots["plot_files"].append(str(heatmap_file))
                interactive_plots["plot_descriptions"]["correlation_heatmap"] = "Interactive correlation heatmap with zoom and hover capabilities"
            ...
            if len(numeric_cols) > 0:
                ...
                fig = make_subplots(
                    rows=2, cols=2,
                    subplot_titles=numeric_cols[:4].tolist(),
                ...
                fig.update_layout(
                    title="Interactive Distribution Plots",
                    height=600,
                    showlegend=False
                )
                
                dist_file = plots_dir / "interactive_distributions.html"
                fig.write_html(str(dist_file))
                interactive_plots["plot_files"].append(str(dist_file))
                interactive_plots["plot_descriptions"]["distributions"] = "Interactive distribution plots with zoom and pan capabilities"
            ...
            if len(numeric_cols) > 1:
                ...
                scatter_file = plots_dir / "interactive_scatter_matrix.html"
                fig.write_html(str(scatter_file))
                interactive_plots["plot_files"].append(str(scatter_file))
                interactive_plots["plot_descriptions"]["scatter_matrix"] = "Interactive scatter plot matrix for feature relationships"
            
            # 4. Interactive Box Plots
            if len(numeric_cols) > 0:
                fig = px.box(
                    df[numeric_cols[:6]].melt(),
                    x="variable", y="value",
                    title="Interactive Box Plots for Outlier Detection",
                    height=600
                )
                fig.update_xaxes(tickangle=45)
                
                box_file = plots_dir / "interactive_box_plots.html"
                fig.write_html(str(box_file))
                interactive_plots["plot_files"].append(str(box_file))
                interactive_plots["plot_descriptions"]["box_plots"] = "Interactive box plots for outlier analysis"
            
            # 5. Interactive Target Analysis
            if target_column and target_column in df.columns:
                ...
                if target_series.dtype in ['object', 'category'] or target_series.nunique() < 10:
                    ...
                    fig = px.histogram(
                        df, x=target_column,
                        title=f"Interactive Target Distribution: {target_column}",
                        height=500
                    )
                else:
                    # Numeric target
                ...
                target_file = plots_dir / "interactive_target_analysis.html"
                fig.write_html(str(target_file))
                interactive_plots["plot_files"].append(str(target_file))
                interactive_plots["plot_descriptions"]["target_analysis"] = f"Interactive analysis of target variable: {target_column}"
            
            return interactive_plots
            
        except Exception as e:
            self.logger.error(f"Error generating interactive visualizations: {e}")
            return {"error": str(e)}

    async def _perform_advanced_statistical_analysis(self, df: pd.DataFrame, target_column: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 1. Normality Tests
            for col in numeric_cols:
                if df[col].notna().sum() > 3:  # Need at least 3 non-null values
                    data = df[col].dropna()
                    ...
                    try:
                        ...
                        analysis["normality_tests"][col]["dagostino"] = {
                    ...
                    try:
                        ...
                        analysis["normality_tests"][col]["kolmogorov_smirnov"] = {
                            "statistic": ks_stat, "p_value": ks_p
                        }
                    except:
                        pass
            
            # 2. Correlation Analysis with Statistical Tests
            if len(numeric_cols) > 1:
            ...
            for col in numeric_cols:
                if df[col].notna().sum() > 10:
                    ...
                    if best_fit:
                        analysis["distribution_fitting"][col] = best_fit
            
            # 4. Generate Statistical Insights
            insights = []
            
            # Normality insights
            normal_cols = [col for col, tests in analysis["normality_tests"].items() 
                          if tests.get("is_normal", False)]
            if normal_cols:
                insights.append(f"✅ {len(normal_cols)} columns appear normally distributed: {normal_cols}")
            else:
                insights.append("⚠️ No columns appear normally distributed - consider transformations")
            
            # Correlation insights
            sig_corrs = analysis["correlation_tests"].get("significant_correlations", [])
            if sig_corrs:
                insights.append(f"🔗 Found {len(sig_corrs)} significant correlations between features")
            else:
                insights.append("ℹ️ No significant correlations found between features")
            
            # Distribution fitting insights
            fitted_dists = analysis["distribution_fitting"]
            if fitted_dists:
                insights.append(f"📊 Successfully fitted distributions for {len(fitted_dists)} columns")
            
            analysis["statistical_insights"] = insights
    ...
    async def _generate_automated_insights(self, basic_stats: Dict, target_analysis: Dict, 
                                         distribution_analysis: Dict, correlation_analysis: Dict,
                                         missing_analysis: Dict, outlier_analysis: Dict,
                                         relationship_analysis: Dict, statistical_analysis: Dict) -> Dict[str, Any]:
        """Generate automated insights using LLM-like analysis"""
        try:
            insights = {
            ...
            n_rows, n_cols = basic_stats.get("shape", (0, 0))
            insights["data_characteristics"].append(f"Dataset contains {n_rows:,} rows and {n_cols} columns")
            
            # Check data size implications
            if n_rows < 1000:
                insights["data_characteristics"].append("Small dataset - consider data augmentation or simpler models")
            elif n_rows > 100000:
                insights["data_characteristics"].append("Large dataset - consider sampling for initial analysis")
            ...
            if high_missing:
                insights["quality_assessment"].append(f"⚠️ High missing values in: {', '.join(high_missing)}")
            else:
                insights["quality_assessment"].append("✅ No columns with excessive missing values")
            
            # 3. Feature Insights
            numeric_cols = [col for col in basic_stats.get("columns", []) 
                           if col in distribution_analysis.get("numeric_distributions", {})]
            
            if numeric_cols:
                skewed_cols = []
                for col in numeric_cols:
                    stats = distribution_analysis["numeric_distributions"].get(col, {})
                    if abs(stats.get("skewness", 0)) > 1:
                        skewed_cols.append(col)
                
                if skewed_cols:
                    insights["feature_insights"].append(f"📊 Highly skewed features: {', '.join(skewed_cols)} - consider log transformation")
                else:
                    insights["feature_insights"].append("✅ Most numeric features are reasonably distributed")
            
            # 4. Modeling Recommendations
            target_dist = target_analysis.get("distribution", {})
            if target_dist.get("class_balance", {}).get("is_balanced", True):
                insights["modeling_recommendations"].append("✅ Target classes are balanced - standard classification metrics are appropriate")
            else:
                insights["modeling_recommendations"].append("⚠️ Class imbalance detected - consider stratified sampling and appropriate metrics")
            
            # Check for high correlations
            high_corrs = correlation_analysis.get("high_correlations", [])
            if high_corrs:
                insights["modeling_recommendations"].append(f"🔗 High correlations detected - consider feature selection or dimensionality reduction")
            
            # 5. Next Steps
            insights["next_steps"].append("1. Address missing values using appropriate imputation strategies")
            insights["next_steps"].append("2. Apply feature scaling for numeric variables")
            insights["next_steps"].append("3. Encode categorical variables appropriately")
            insights["next_steps"].append("4. Consider feature engineering based on domain knowledge")
            
            if statistical_analysis.get("normality_tests"):
                normal_cols = [col for col, tests in statistical_analysis["normality_tests"].items() 
                              if tests.get("is_normal", False)]
                if not normal_cols:
                    insights["next_steps"].append("5. Consider data transformations for non-normal features")
